...
if $(xrandr | grep -q "^eDP-1 connected") && $(xrandr | grep -q "^DP-2-1 connected"); then
  DP2_res=$(xrandr | grep -A1 "^DP-2-1 connected ." | tail -n +2 | sed 's/^\s\+\([0-9x]\+\).\+/\1/')
  xrandr --output DP-2-1 --primary --mode $DP2_res --refresh 100 --pos 1920x0 --rotate normal --output eDP-1 --mode 1920x1080 --pos 0x0 --rotate normal
